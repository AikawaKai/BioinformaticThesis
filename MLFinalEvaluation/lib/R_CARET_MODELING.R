...
caret.modeling.fs.cor.based <- function(net.dir=net.dir, net.file=net.file, ann.dir=ann.dir, 
	...
	if(cfs){
	  ...
	  file_name <- paste0(scores.dir, "Scores.", out.name,  "PCA.", nfeature, ".", algorithm, ".", kk, "fcv.rda")
	  save(S, file=file_name, compress=TRUE);
	  file_name2 <- paste0(perf.dir, "PerfMeas.", out.name, "PCA.", nfeature, ".", algorithm, ".", kk, "fcv.rda")
	...
	write.csv(x = csv_result, file=csv_name, row.names = FALSE)
}
